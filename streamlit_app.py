...
def main():
    ...
    **Interpretatie van de kaart:**
    ...
    - 🔵 Punten tonen individuele woningen met splitsingskans
    - 🟣 Paarse punten tonen geplande woningbouwprojecten

    Ontwikkeld door [Ruben Woudsma](https://rubenwoudsma.nl)
    """)

    st.info(
        "Dit model is indicatief. Het laat zien waar potentieel zit, niet wat direct gerealiseerd kan worden."
    )
    ...
    k1.metric(
        "Kandidaat-adressen",
        len(candidates),
        help="Aantal woningen groter dan de gekozen drempel"
    )

    k2.metric(
        "Verwachte extra woningen",
        int(round(candidates["expected_units_added"].sum())),
    ...
    k3.metric(
        ...
        help="Aantal buurten met splitsingspotentieel"
    )
    ...
    if len(buurten):
        ...
        merged["expected_units_added"] = merged["expected_units_added"].round(0)
    ...
    st.caption(
        "Bij in- en uitzoomen kan de kaart kort verversen."
    )
